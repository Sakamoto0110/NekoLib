...
namespace NekoLib.Navigation.Tests.Unit.Fakes
{
    ...
    internal sealed class RuntimeTestFixture
    {
        ...
        /// <summary>
        /// Builds a runtime registered with <typeparamref name="THome"/> as Home,
        ...
        /// </summary>
        public static RuntimeTestFixture Build<THome>(params Type[] otherPageTypes)
            where THome : StubPageView, new()
        {
            ...
            var registry = PageRegistry.Create(builder =>
            {
                ...
                builder.RegisterType(typeof(THome), d => d.Role = PageRole.Home);
            ...
            var factory = PageFactory.AutoWireFromRegistry(EnumerateRegisteredTypes<THome>(otherPageTypes));
        ...
        private static Type[] EnumerateRegisteredTypes<THome>(Type[] others)
        {
