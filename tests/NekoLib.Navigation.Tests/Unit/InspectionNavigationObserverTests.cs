...
namespace NekoLib.Navigation.Tests.Unit
{
    ...
    /// to an <see cref="IInspectionRecorder"/> sink as recorded operations + a pull-based
    /// "current" state, and that disable / dispose are honoured. Uses the real
    /// <see cref="InspectionRuntime"/> so the wiring is exercised end-to-end.
    /// </summary>
    public sealed class InspectionNavigationObserverTests
    {
        [Fact]
        public void SuccessfulNavigation_RecordsNavigatedOperation()
        {
            ...
            using (InspectionNavigationObserver.Attach(hub, debug))
            {
                hub.Publish(SuccessEntry());

                var op = Assert.Single(debug.GetOperations());
                Assert.Equal(InspectionNavigationObserver.Module, op.Module);
        ...
        public void Dispose_DetachesFromHub()
        {
            ...
            var handle = InspectionNavigationObserver.Attach(hub, debug);
            handle.Dispose();
        ...
        [Fact]
        public void DisabledInspection_AttachIsNoOpAndDoesNotThrow()
        {
            var hub = new NavigationEventHub();

            using (InspectionNavigationObserver.Attach(hub, NullInspection.Instance))
            {
        ...
        {
            var hub = new NavigationEventHub();
            var debug = new InspectionRuntime(new InspectionOptions { Capacity = 1 });
        ...
        public void ProviderRegistrationFailure_RollsBackPartialObserver()
        {
            ...
            var existing = debug.RegisterStateProvider(
                InspectionNavigationObserver.Module,
                "current",
                () => "existing");
            ...
            try
            {
                Assert.Throws<InvalidOperationException>(
                    () => InspectionNavigationObserver.Attach(hub, debug));
        ...
        public void Dispose_UnregistersStateProviders()
        {
            ...
            InspectionNavigationObserver.Attach(hub, debug).Dispose();

            Assert.Empty(debug.StateKeys());
        }

        private static object Stats(InspectionRuntime debug)
            => debug.CaptureState()[InspectionNavigationObserver.Module + "::stats"];
