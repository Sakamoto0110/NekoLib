...
namespace NekoLib.Navigation.Tests.Unit
{
    ...
    public sealed class InspectionGlobalBootstrapTests
    {
        [Fact]
        public async Task UseInspection_GlobalEnabledAtStart_ShutdownRemovesContextProviders()
        {
            ...
            using var debug = InspectionRuntime.EnableGlobal();
            try
            {
                bootstrap.Start();
        ...
        [Fact]
        public async Task UseInspection_GlobalNoOp_StartsWithoutObserver()
        {
            var registry = PageRegistry.Create(
                builder => builder.RegisterType(typeof(StubIdle)));
            try
