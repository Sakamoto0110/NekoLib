...
**Subject:** synchronous logging pipeline composition, ownership, delivery ordering, snapshots, flush and disposal contracts, and the shipped sinks
...
`ILogFlusher`, `ILogSnapshotSource`, `LogEntry`, `LogLevel`, `NullLogger` — and
the `LoggerExtensions` severity helpers. `Logger` itself exposes only `Log`, so
the `logger.Info(...)` call below is one of those Core extensions, not a member
of this package. This document owns what the concrete pipeline and the shipped
sinks actually do.
...
- the budget is **one total**, spent on lock admission first and then on the
  sinks in registration order. A flush that waits on the pipeline gate has
  already consumed part of the budget the sinks were going to share;
...
That unbounded flush runs **under the pipeline lock**, so it also blocks
`GetRecentEntries` for its duration. An incident collector that reads the recent
window while shutdown is stuck in a sink waits with no budget of its own; see
[`LOG-FINDING-001`](FINDINGS.md).
...
stream. `Write(null)` throws `ArgumentNullException`. A trace listener that
throws is swallowed inside the sink, so a broken listener cannot become a
pipeline failure.
...
Bounded file persistence. Each `Write` creates the target directory if it is
missing, opens the file, appends one line, flushes and closes, so the sink holds
no handle between writes and is not `IDisposable`. Construction normalizes and
validates the path but never touches the filesystem, so an unwritable location
first surfaces as an absorbed sink failure on the first entry, not as a
...
the two accepted manifests are identical and every behavior above applies
equally to `net481` and `net9.0`. The project defines `TRACE` for both build
configurations, which is what keeps the Release `DebugLogSink` from compiling
itself away.

Sustained and fault-driven behavior comes from the shared Observability
scenario at
[`runtime_tests/Observability/LongRunningRecovery/`](../../../runtime_tests/Observability/LongRunningRecovery/README.md).
That scenario also drives Telemetry and Inspection; only its Logging checks are
Logging evidence.
