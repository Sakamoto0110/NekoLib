...
**Subject:** migration from the initial Core candidate surface to the accepted F1-CORE contract
...
[Core public API review](../audits/public-api-review-2026-08-17.md). The current
contract is owned by the [Core reference](../REFERENCE.md).
...
scope while accepting `NEKOEXP0001`. The dependent `NekoLib.Inspection`
action family now carries the same experiment identity; no feature module
registers an action. Later evolution must follow the current Core and
Inspection references plus the public API policy.
...
- Inspection passive recording, state providers, snapshots, null behavior, and
  singleton-capable provider lifecycle remain stable contracts;
