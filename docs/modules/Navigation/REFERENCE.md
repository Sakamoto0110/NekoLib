...
**Subject:** normative technical contract for the NekoLib.Navigation family
...
| Package | Targets | Direct project dependency | Role |
|---|---|---|---|
| `NekoLib.Navigation` | `net481`, `net9.0` | `NekoLib.Core` | Framework-neutral contracts, metadata, bootstrap, runtime, history, session, surfaces, diagnostics, and toolkit abstractions |
| `NekoLib.Navigation.WinForms` | `net481`, `net9.0-windows` | `NekoLib.Navigation` | WinForms native host, page/surface bases, event, timer, focus, interaction, blocking, loading-mask, and toolkit adapters |
| `NekoLib.Navigation.Wpf` | `net481`, `net9.0-windows` | `NekoLib.Navigation` | WPF equivalents over `Panel`, `Dispatcher`, routed focus/input, and WPF lifetime rules |

The core package references `Microsoft.Bcl.AsyncInterfaces` only for its
`net481` asset. The two platform packages do not reference each other; a
consumer normally installs one adapter package and receives Navigation and Core
transitively. All three projects are shipped libraries and members of
`NekoLib.sln`. Their six accepted public API manifests are the stable compiled
surface oracle and are semantically aligned across each package's two targets;
target-framework attributes remain target-specific.
...
The shipped WinForms and WPF page and surface bases have protected
constructors and are non-abstract so Visual Studio can instantiate a consumer
subclass on a native design surface. WinForms dialog, prompt, and popover bases
skip runtime parent fixup in design mode and defer runtime layout work until a
window handle exists; parenting them before handle creation must not call
`BeginInvoke` prematurely.

One limitation remains part of the consumer contract: the WinForms designer
cannot open a type whose direct base remains generic. A typed prompt therefore
uses a non-generic consumer shim that closes the result type:
...
This shim does not change the runtime prompt result or ownership contract.
Removing it would require a public redesign of the typed prompt surface and is
not scheduled. Do not apply `[DesignerCategory("Code")]` to a page or surface
that should open visually; that attribute deliberately routes Visual Studio to
the code editor.
...
## Cancellation and operation admission

Navigation exposes no caller `CancellationToken`. A switch admitted before the
shutdown cutoff owns a runtime lease through UI dispatch, gate execution, and
its synchronous lifecycle; calls arriving after the cutoff fail instead of
queueing against a disappearing runtime. Dialog, prompt, and popover admission
ends once the service has registered ownership of the surface, so shutdown can
close a still-pending modal without waiting for user completion.

Guard evaluation has a fixed 30-second admission bound, but `IGuard` itself has
no cancellation parameter. A timeout denies the attempt and releases the
navigation gate; it cannot forcibly stop the guard's application-owned task, so
guard implementations must bound and clean up their own work.

`IBackgroundLoadable` likewise has no cancellation parameter. Reset, rollback,
page replacement, and shutdown cancel the runtime wrapper's interest: they
discard the result, stop waiting, close the loading mask, and observe any late
task failure. They do not abort arbitrary page-owned work. A loader that owns
I/O, timers, or other resources must provide its own bounded lifetime and make
late completion safe; only `ApplyBackgroundResultAsync` is protected from
running after the page is no longer the live current page.
...
## Security and sensitive application state

Navigation is an in-process coordination library, not an authentication,
authorization, isolation, or serialization boundary. `NavigationSession`
stores application-supplied role and permission strings in memory; built-in
guards evaluate those strings, and `[AllowAnonymous]` deliberately bypasses the
descriptor guard. The application still owns credential acquisition, identity
proof, secure storage, and every policy decision outside those local checks.

Navigation does not serialize or persist `NavigationArgs.Payload`, captured
page state, page instances, or surface payloads. History retains captured state
objects in memory until the entries are cleared, reset, or shutdown, so page
authors must not treat the stack as protected storage. Scalar tracing and
Inspection snapshots omit payloads, page instances, captured state, role names,
and permission names. Logging and public diagnostics do expose logical page
names/types, guard-denial reasons, failure types, and exception messages; custom
guards and pages must not place credentials or secrets in those strings, and a
configured writer remains responsible for its own retention and access policy.
...
> Diagnostics read-only snapshot bridge. Broad Inspection instrumentation in
