...
|---|---|---|---|
| Data / FarmDatabase | Same application code against SQLite and Access: row limiting, parameter binding, catalog discovery, DDL dialect, transactional stock movements with an audit row, and a reason-collecting prompt. Also the only exercise of `NekoLib.Mvvm` through a real binding surface and of attribute-only page registration | Interactive pass on `net9.0-windows` on 2026-08-05 at `f169ab6` — full sweep on SQLite, core paths on Access; `net481` builds but has not been driven. Designer loadability verified on 2026-08-06 at `73ddbdb` | [`Data/FarmDatabase/README.md`](Data/FarmDatabase/README.md) |
...
| Navigation / WinForms smoke | WinForms page switching, idle, guards, overlays, interaction blocker, and shutdown on both target families | Partial interactive pass on `net9.0-windows` on 2026-08-03 at `4ab9629`: step 5 driven and passing; `net481` builds but has not been driven | [`Navigation/WinFormsSmoke/README.md`](Navigation/WinFormsSmoke/README.md) |
