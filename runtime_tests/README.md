...
| Navigation / long-running and recovery (E3-NAV) | Shared unattended workload with minimal native WinForms/WPF hosts: page lifetime and history, session guards, all load modes, surfaces, idle, repeated mount/awaited shutdown, admitted/cutoff races, 14 scenario-owned failures, passive Inspection, and resource cleanup | **Qualifying standalone smoke passed on all three combinations, 2026-08-11.** Clean `897e17b` ran WinForms `net481`, WinForms `net9.0-windows`, and WPF `net9.0-windows` for 20 minutes each. Every run passed 11/11 checks with zero failed/skipped, exit 0, awaited shutdown, zero cleanup problems/native children, and no remaining process/window; they completed 130,919, 131,717, and 2,198,271 operations. The three combinations remain distinct, disabled E3-ORCH workers because campaign preflight cannot prove an interactive desktop. WPF private bytes rose during the smoke while handles oscillated, managed heap was non-monotonic, and owned state returned to zero; retain this as a soak observation rather than a leak finding. Recovery rehearsals, one four-hour native run, automated UI, interactive parity, and long-window resource interpretation remain pending | [`Navigation/LongRunningRecovery/README.md`](Navigation/LongRunningRecovery/README.md) |
