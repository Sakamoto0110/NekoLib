...
| Data / SQL Server (E4-SQL) | `NekoLib.Data` against a real server engine in an adopted local container: pooling and ownership, mid-flight cancellation, transport loss and recovery, transactions, read shapes and mapping, streaming cleanup, and `DynamicMode.IL` schema-cap lifetime | **Automated runtime, 2026-08-08.** `--smoke` exited 0 on both target frameworks against SQL Server 16.0.4265.3 in the pinned container — 28 checks on `net9.0`, 27 plus a correctly skipped streaming check on `net481` — with cleanup reconciled and the container restored. A 10-minute recovery rehearsal also exited 0 with all seven fault handlers passing, but it is below the specified 60–90 minute window and is flagged as such. This is the first real-server provider evidence in the repository; the remaining rehearsal and soak work is recorded in the scenario | [`Data/SqlServer/README.md`](Data/SqlServer/README.md) |
