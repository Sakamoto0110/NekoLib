...
| HTTP / unofficial TheCatAPI provider probe | Typed GET/POST/DELETE catalog over the public provider: image search/lookup plus run-owned favourite creation, query, deletion and bounded reconciliation | Build passed on `net481` and `net9.0` with 0 warnings on 2026-08-16. The missing-key path exited `3` on both targets without sending a request and finalized sanitized, cleanup-complete artifacts. No provider request has been executed; a maintainer-owned API key is required | [`Http/TheCatApi/README.md`](Http/TheCatApi/README.md) |
