...
| HTTP / unofficial TheCatAPI provider probe | Typed GET/POST/DELETE catalog over the public provider: image search/lookup plus run-owned favourite creation, query, deletion and bounded reconciliation | **Provider gate complete, 2026-08-16.** Builds passed on both targets with 0 warnings. Missing-key executions exited `3` without requests. Real-provider runs then passed 10/10 with exit `0` on `net9.0` and `net481`: search/lookup, run-owned favourite creation/query/deletion, post-delete absence and final reconciliation all passed with zero cleanup problems and sanitized artifacts | [`Http/TheCatApi/README.md`](Http/TheCatApi/README.md) |
