...
| Watchdog / deployed-Host crash and recovery (E3-WDOG) | Independent controller, scenario application child, and separately deployed `NekoLib.Watchdog.Host`: bounded attach, replacement generations, ordinary and unhandled terminals, fast-crash cooling, paused clean shutdown, Host/bootstrap restart, forwarding, bundle integrity/retention, exact process ownership, and cleanup | **Build-only, 2026-08-10.** All three modes and six fault kinds are implemented for `net481` and `net9.0-windows`; both targets build, 6/6 isolated contract checks pass per target, and recovery seed `20260810` is stable across repeated runs and targets (`fnv1a64:677fcab193b16fbf`). **No child or Host has ever been started by this scenario.** Source layout is explicitly not package evidence; package-backed execution remains pending and no package was created for this delivery | [`Watchdog/CrashRecovery/README.md`](Watchdog/CrashRecovery/README.md) |
