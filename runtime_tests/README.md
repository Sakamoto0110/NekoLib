...
| Devices / com0com serial parity and recovery (E3-DEV) | Two mutually exclusive paths over the same pairs. **Oracle pass:** real virtual-COM configuration, timeout/cancellation, read modes, reopen, and PCB-A text and PCB-B binary parity against an independent emulator. **E3-DEV:** the scenario owns the other end of each pair and drives open/close/reopen cycles, timeouts, cancellation, endpoint switching, operation serialization, chunked and late responses, configuration and encoding parity, disposal under load, and five peer faults | **Oracle: interactive pass** on `net481` and `net9.0` on 2026-08-01 from working trees based on NekoLib `628442a` and emulator `9c9528f`; middleware behavior also confirmed visually by the user; repeat after immutable commits. **E3-DEV development probes, 2026-08-11.** The real COM preflight proved both pairs. After fixing two scenario defects found by the first `net9.0` run, the same two-minute command exited 0 on both targets: 168/168 checks, 0 skipped, 11 cycles in 124 seconds, and all four ports reopened and released. Both targets build with no scenario warning and the recovery schedule remains hash-stable (`fnv1a64:7496700bf4b75339`). These probes are below the 15-minute smoke minimum; full smoke, recovery and soak remain open. The two paths cannot run at once: both want `COM9`/`COM10` | [`Devices/Com0Com/README.md`](Devices/Com0Com/README.md) |
| Observability / Logging, Telemetry, Inspection (E3-OBS) | The three opt-in capabilities composed as an application composes them, each with independent assertions and its own result section: ordering and bounded snapshots, sink-failure isolation, rotation and retention, bounded flush, telemetry retention and correlation, Inspection provider budgets, and process-wide slot lifecycle | **Automated runtime, 2026-08-09.** `--smoke` exited 0 on both target frameworks over the specified 15-minute window — 4951 checks on `net9.0` and 4591 on `net481`, **zero failed and zero skipped on either**, across 164 and 152 cycles and roughly 1.8M and 1.6M operations with no unexpected failure. Threads and handles stayed flat and every bounded structure ended at capacity. The recovery rehearsal then passed inside the specified window on **both** targets — 62.3 minutes each, exit 0, 68 checks, **all seven fault kinds proven**, zero unexpected failures, and byte-identical counters across the two, so nothing is skipped or divergent on either. Ctrl+C on a soak exits 8 with cleanup reconciled. It needs no container, service or hardware, and every failure it injects is scenario-owned. **The soak path is proven**: `--soak 15m` exited 0 with 3848 checks and 128 cycles, and all seven fault kinds fired while the assertion cycles were running. The remaining full-duration gate is 4 hours | [`Observability/LongRunningRecovery/README.md`](Observability/LongRunningRecovery/README.md) |
| Pipes / long-running and recovery (E3-PIPE) | `NekoLib.Pipes` across real process boundaries over real named pipes: payload sizes, concurrent correlation, error contracts, frame limits, malformed raw peers, endpoint release and rebinding | **Automated development probes, 2026-08-11.** After fixing one product liveness defect and two scenario defects found by the first runs, the same short standalone command passed on both targets: four cycles, 75/75 checks, 0 skipped, exit 0 in 133 seconds, server child exit 0 and endpoint released. Both targets build with zero warnings and the recovery schedule remains hash-stable (`fnv1a64:42db44086ce556a2`). These runs are deliberately below the specified smoke window; full smoke and rehearsal on both targets plus one four-hour soak remain open. The scenario is not registered in E3-ORCH | [`Pipes/LongRunningRecovery/README.md`](Pipes/LongRunningRecovery/README.md) |
| Watchdog / deployed-Host crash and recovery (E3-WDOG) | Independent controller, scenario application child, and separately deployed `NekoLib.Watchdog.Host`: bounded attach, replacement generations, ordinary and unhandled terminals, fast-crash cooling, paused clean shutdown, Host/bootstrap restart, forwarding, bundle integrity/retention, exact process ownership, and cleanup | **Passing development probes, 2026-08-11.** Both targets build; 7/7 isolated scenario contracts and 84/84 Watchdog tests pass per target. After one narrowly authorized product fix preserved the initially attached process's exit code and two `net481` scenario defects were corrected, final two-minute source-layout probes passed identically on both targets: smoke hash `fnv1a64:7d3f49941df33843`, all six faults, seven healthy generations, 20/20 checks, zero skipped, exit 0 in about 123 seconds, finalized bundle/artifacts, released endpoints and no remaining process. No API, protocol, bootstrap, restart policy or shared harness changed. These runs are below the 15-minute smoke minimum and source layout is not package evidence; full smoke/rehearsal, package-backed proof and the four-hour soak remain open. No package was created, and the scenario remains out of E3-ORCH | [`Watchdog/CrashRecovery/README.md`](Watchdog/CrashRecovery/README.md) |
| Navigation / long-running and recovery (E3-NAV) | Shared unattended workload with minimal native WinForms/WPF hosts: page lifetime and history, session guards, all load modes, surfaces, idle, repeated mount/awaited shutdown, admitted/cutoff races, 14 scenario-owned failures, passive Inspection, and resource cleanup | **Passing development probes, 2026-08-11.** After correcting three scenario assumptions found by the first `net481` execution, clean `e7c86a4` passed the same two-minute standalone command on WinForms `net481`, WinForms `net9.0-windows`, and WPF `net9.0-windows`: 11/11 checks, zero skipped, exit 0, awaited shutdown, zero cleanup problems, and no remaining process/window in every combination. The runs completed 50, 51, and 552 cycles respectively. Builds and deterministic previews remain green with no warning from the scenario. The three combinations are registered as distinct, disabled E3-ORCH workers; their build + preflight and 42-event aggregate recovery schedule passed without launching a worker. They remain opt-in because they require an interactive desktop and the probes are below the 15-minute smoke minimum. Full smoke, recovery rehearsal, soak, automated UI, interactive parity, and resource baselines remain pending | [`Navigation/LongRunningRecovery/README.md`](Navigation/LongRunningRecovery/README.md) |
