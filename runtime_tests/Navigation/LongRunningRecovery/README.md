...
**Last verification:** 2026-08-10 — **build-first only.** The three required
target/platform combinations build, the six isolated contracts pass on both
core targets, and recovery seed `20260810` produces
`fnv1a64:2af9145c9ebf63ec` on repeated previews in every host build. Seed `99`
produces `fnv1a64:838221b8dec2baa0`. No Navigation workload, native UI smoke,
recovery rehearsal, soak, automated UI, or interactive procedure has run.
...
Start with one standalone WinForms `net481` smoke. Register no campaign worker
until that run exits 0 and its artifacts and cleanup have been inspected:

```powershell
dotnet run --project runtime_tests\Navigation\LongRunningRecovery\NekoLib.Navigation.RuntimeTests.LongRunningRecovery.WinForms\NekoLib.Navigation.RuntimeTests.LongRunningRecovery.WinForms.csproj -f net481 -- --smoke
```

Then run the other two standalone combinations with the same mode. Only after
all three smokes pass should the recovery rehearsals run, followed by one
four-hour full native combination and smoke/rehearsal parity on the other two.
Do not infer WPF behavior from WinForms evidence or one target from another.
